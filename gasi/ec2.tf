# EC2 인스턴스 프로필
resource "aws_iam_instance_profile" "gasi" {
  name = "request-${terraform.workspace}.gasi.profile"
  role = aws_iam_role.gasi.name
}

# EC2 인스턴스 보안 그룹
resource "aws_security_group" "gasi" {
  name   = "request-${terraform.workspace}.gasi.sg"
